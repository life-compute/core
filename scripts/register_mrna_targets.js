...
const PROGRAM_ID = flag("--program", "74RHjg1zYgN9zuVykde4SK2ERiRgNkouATW9MmQDLRWf");
const DRY_RUN    = hasFlag("--dry-run");
const DELAY_MS   = 800;   // rate-limit between txs

const KEYPAIR_PATH = flag("--keypair",
...
if (!TARGETS_PATH) { console.error("targets.json not found"); process.exit(1); }
...
async function withRetry(fn, label, max = 3) {
  for (let a = 1; a <= max; a++) {
    try { return await fn(); }
      ...
      console.log(`    retry ${a}/${max-1} for ${label} in ${wait}ms — ${e.message?.slice(0,80)}`);
      await sleep(wait);
    }
  }
}
...
async function main() {
  ...
  const pdas = mrnaTargets.map((_, i) => {
    const buf = Buffer.alloc(2); buf.writeUInt16LE(MRNA_BASE_ID + i, 0);
    return web3.PublicKey.findProgramAddressSync([Buffer.from("target"), buf], programId)[0];
  });
  const infos = await connection.getMultipleAccountsInfo(pdas);
  const toRegister = [], alreadyDone = [];
  ...
  if (alreadyDone.length) {
    console.log("Already done:", alreadyDone.map(x => `${x.id}(${x.onchainId})`).join(", "));
    console.log();
  }
  ...
  if (DRY_RUN) {
    ...
    for (const { onchainId, t } of toRegister) {
      console.log(`  [${onchainId}] ${t.id.padEnd(20)} uniprot=${t.uniprot_id}  diff=Hard`);
    }
    return;
  }
    ...
    try {
      const tx = await withRetry(async () => {
        ...
        const msg = new web3.TransactionMessage({
          payerKey: authKp.publicKey, recentBlockhash: bh.blockhash, instructions: [ix],
        }).compileToV0Message();
  ...
  console.log(`Registered: ${registered}  |  Already existed: ${alreadyDone.length}  |  Failed: ${failed}`);
  if (failures.length) {
    console.log("\nFailed:");
    failures.forEach(f => console.log(`  [${f.onchainId}] ${f.id}: ${f.error}`));
  }
  if (registered > 0) console.log(`\n✓ ${registered} mRNA target(s) registered on-chain at IDs 2000-${2000+registered-1}.`);
}
