...
(async () => {
  ...
  const authKp = Keypair.fromSecretKey(
    Buffer.from(JSON.parse(fs.readFileSync(args.authKeypair))),
  );
  const minerKp = Keypair.fromSecretKey(
    Buffer.from(JSON.parse(fs.readFileSync(args.minerKeypair))),
  );
  ...
  const endSlot =
    (config.epochStartSlot?.toNumber?.() || 0) +
    (config.epochDurationSlots?.toNumber?.() || 0);
  log(
    "currentSlot:",
    currentSlot,
    "| epochEndsAt:",
  ...
  const [minerAccountPda] = PublicKey.findProgramAddressSync(
    [SEED_MINER, minerKp.publicKey.toBuffer()],
    PROG_ID,
  );
  ...
  log(
    "resultPda on-chain:",
    resultInfo !== null ? `YES (${resultInfo.data.length} bytes)` : "NO",
  );
  ...
  if (resultInfo !== null) {
    ...
    try {
      ...
      log(
        ...
        "confirmedCount=" +
          (existing.confirmedCount ?? existing.confirmed_count),
        "submittedSlot=" +
          (existing.submittedSlot?.toString() ??
            existing.submitted_slot?.toString()),
        "smilesLen=" + smilesLen,
        "smiles=" + smilesStr,
      ...
      log(
        "WARN: could not decode existing resultSubmission:",
        decodeErr.message,
      );
    }
    process.stdout.write(
      JSON.stringify({ status: "already_submitted", epoch: epoch.toString() }) +
        "\n",
    );
    process.exit(0);
  }
  ...
  log(
    "minerAccount on-chain:",
    minerInfo !== null ? "YES" : "NO (will register)",
  );
  if (minerInfo !== null) {
    ...
    try {
      ...
      log(
        "minerAccount submissionCount:",
        submissionCount,
        "| submissionEpoch:",
        submissionEpoch.toString(),
        "| currentEpoch:",
        epoch.toString(),
      );
      if (
        submissionEpoch.toString() === epoch.toString() &&
        submissionCount >= 3
      ) {
        log(
          "WARN: submission rate limit reached for this epoch (count=" +
            submissionCount +
            ")",
        );
      }
      ...
      if (
        errMsg.includes("AccountDidNotDeserialize") ||
        ...
        log(
          "STALE MinerAccount detected — layout mismatch with current program. Triggering re-registration.",
        );
        process.stdout.write(
          JSON.stringify({ status: "stale_miner_account", reason: errMsg }) +
            "\n",
        );
        process.exit(0);
      }
  ...
  if (minerInfo === null) {
    try {
      ...
      log(
        "registerMiner explorer: https://explorer.solana.com/tx/" +
          t +
          "?cluster=devnet",
      );
    } catch (regErr) {
  ...
  if (jobInfo === null) {
    ...
    const [tgt] = PublicKey.findProgramAddressSync(
      [SEED_TARGET, targetIdBuf],
      PROG_ID,
    );
    ...
    log(
      "target PDA on-chain:",
      tgtInfo !== null
        ? `YES (${tgtInfo.data.length} bytes)`
        : "NO (target not registered!)",
    );
    try {
      const t = await program.methods
        .assignJob(TARGET_ID_NUM, SEQ)
        .accounts({
      ...
      log("assignJob tx:", t);
      log(
        "assignJob explorer: https://explorer.solana.com/tx/" +
          t +
          "?cluster=devnet",
      );
    } catch (jobErr) {
  ...
  try {
    ...
    const allLogs = (submitErr.logs || [])
      .concat(submitErr.errorLogs || [])
      .join("\n");
    if (
      submitErrMsg.includes("AccountDidNotDeserialize") ||
      allLogs.includes("AccountDidNotDeserialize") ||
      (submitErr.error &&
        JSON.stringify(submitErr.error).includes("AccountDidNotDeserialize"))
    ) {
      log(
        "STALE MinerAccount detected during submitResult — triggering re-registration.",
      );
      process.stdout.write(
        JSON.stringify({
          status: "stale_miner_account",
          reason: submitErrMsg,
        }) + "\n",
      );
      process.exit(0);
  ...
  log(
    "submitResult explorer: https://explorer.solana.com/tx/" +
      submitTx +
      "?cluster=devnet",
  );
  ...
  try {
    ...
    if (confirmResult.value.err) {
      log(
        "ERROR: tx confirmed but with error:",
    ...
    log(
      "WARN: confirmTransaction threw (tx may still have landed):",
      confirmErr.message,
    );
  }
  ...
  try {
    const txDetail = await conn.getTransaction(submitTx, {
      commitment: "confirmed",
    ...
    if (txDetail) {
      ...
      log(
        "WARN: getTransaction returned null (not yet available at confirmed commitment)",
      );
    }
  } catch (fetchErr) {
  ...
  log(
    "resultPda after submit:",
    createdInfo !== null
      ? `EXISTS (${createdInfo.data.length} bytes)`
      : "MISSING — tx did not create account!",
  );
